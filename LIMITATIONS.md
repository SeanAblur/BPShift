...
| **Parity capture / diff / drift** (`parity-capture` / `parity-diff` / `drift`) | ✅ partial | `parity-capture` runs `SnapshotBPBehavior` commandlet and produces a `behavior_trace_v1` JSON. `parity-diff` compares two traces in pure Python (no UE editor) using a mirror of `CompareSnapshots` semantics (1e-3 absolute tolerance, recursive). Self-parity verified on 2 real Actor BPs (77 and 81 UPROPERTY fields each): same BP, two runs, `initialValueMismatches: 0`. **Partial because**: (1) only initial UPROPERTY state is tested when using an empty scenario — function-call traces require a safe hand-authored scenario (see Known Limitations); (2) cross-version comparison (e.g. UE 5.2 vs 5.3) is not yet run — see "Not yet verified". `drift` is a thin alias for `parity-diff`. |
...
| Cross-version parity comparison (`parity-diff` across UE 5.2 vs 5.3) | ❌ untested | `parity-diff` itself is pure Python and engine-agnostic; the gap is the `parity-capture` step, which needs the `SnapshotBPBehavior` commandlet built against each engine version separately. Not run against a 5.3 build — no validated drift data yet. |
...
- **`parity-capture` auto-scenario crashes on UI / Slate-dependent functions.** `SnapshotBPBehavior` auto-generates a scenario from all `BlueprintCallable` functions and calls each via `ProcessEvent`. Functions that internally call `FSlateApplication::Get()` (e.g. `SetFocusToGameViewport`, cursor helpers) assert-crash in commandlet mode because Slate is not initialized. **Workaround**: pass `--scenario` with a hand-authored JSON listing only safe functions, or an empty scenario (`{"schema":"scenario_v1","steps":[]}`) to capture initial UPROPERTY state only. Function libraries and macros with no instance state still produce useful parity data even with an empty scenario.
